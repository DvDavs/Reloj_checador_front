...
>(({ className, ...props }, ref) => (
  ...
  >
    <CheckboxPrimitive.Indicator
      forceMount
      className={cn(
        'hidden items-center justify-center text-current group-data-[state=checked]:flex group-data-[state=indeterminate]:flex'
      )}
    >
      <Check className='h-4 w-4 group-data-[state=indeterminate]:hidden' />
      <Minus className='h-4 w-4 group-data-[state=checked]:hidden' />
    </CheckboxPrimitive.Indicator>
  </CheckboxPrimitive.Root>
));
