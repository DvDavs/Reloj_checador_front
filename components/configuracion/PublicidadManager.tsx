  ...
  const handleSaveOrder = async () => {
    ...
    try {
      ...
      toast({
        ...
        description: 'La galería ha sido actualizada.',
      });
    } catch (error) {
          ...
          >
            ...
            {previewUrl ? (
              <div className='flex flex-col md:flex-row items-center gap-6'>
                <div className='relative w-48 aspect-video rounded-lg overflow-hidden shadow-md border border-white/20'>
                  <Image
                    src={previewUrl}
                    alt='Preview'
                    fill
          ...
          {images.length === 0 ? (
            ...
            images.map((item, index) => (
              <motion.div
                ...
                className={cn(
                  'group/item relative rounded-xl overflow-hidden border cursor-grab active:cursor-grabbing select-none hover:shadow-md transition-shadow bg-white',
                  item.active
              ...
              >
                ...
                <div className='relative aspect-video w-full bg-slate-100'>
                  ...
                  {/* Indicadores y Acciones Superiores */}
                  {(item.isDefault || index === 0) && (
                    <div className='absolute top-3 inset-x-0 flex justify-center z-10'>
                      <div className='bg-blue-600/90 backdrop-blur-sm px-2 py-0.5 rounded-full border border-white/40 shadow-lg animate-in fade-in zoom-in duration-300'>
                    ...
                    <div
                      className={cn(
                        ...
                        index === 0 || item.active
                          ...
                          : 'bg-slate-400 text-slate-100 opacity-50'
                      )}
                    >
                  ...
                  <div className='absolute top-3 right-3 flex flex-col items-end gap-2 pointer-events-auto'>
                    {/* Botón Default (siempre visible si lo es) */}
                    <div
                      className={cn(
                        'h-6 w-6 rounded-full border-2 border-white shadow-lg flex items-center justify-center transition-all',
                        index === 0 || item.isDefault
                          ? 'bg-blue-600 text-white'
                          : 'bg-white/80 backdrop-blur-sm text-blue-600 opacity-0 group-hover/item:opacity-100'
                      )}
                    >
                      <CheckCircle2 className='h-3 w-3' />
                    </div>
                  ...
                  {/* Drag Handle Icon */}
                  <div className='absolute bottom-3 right-3 p-1.5 bg-white/50 backdrop-blur-sm rounded-full border border-white opacity-40 group-hover/item:opacity-100 transition-opacity'>
