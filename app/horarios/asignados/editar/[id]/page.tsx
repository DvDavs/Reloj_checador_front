...
function Step2_SelectSchedule({ state, dispatch, setTemplates, isEditMode }: { 
    state: WizardState, 
        ...
        <Card>
            ...
            <CardContent className="space-y-6">
                <RadioGroup
                    ...
                    onValueChange={(value) => dispatch({ type: 'SET_SCHEDULE_SELECTION_TYPE', payload: value as 'existing' | 'new' })}
                    ...
                    <div className="pl-6 pt-4 border-l">
                        {isLoading ? (
                            ...
                            <Select
                                ...
                                onValueChange={(value) => dispatch({ type: 'SELECT_EXISTING_TEMPLATE', payload: parseInt(value) })}
                            >
                                <SelectTrigger>
                    ...
                    <Label>Tipo de Horario</Label>
                     {isLoading ? (
                        ...
                        <Select
                            ...
                            onValueChange={(value) => dispatch({ type: 'SELECT_SCHEDULE_TYPE', payload: parseInt(value) })}
                        >
                            <SelectTrigger>
...
function Step4_Summary({ state, templates, scheduleTypes }: { state: WizardState; templates: HorarioTemplateDTO[], scheduleTypes: TipoHorarioDTO[] }) {
    ...
    return (
        <div className="max-w-2xl mx-auto">
             <h3 className="text-xl font-bold text-center mb-6">Resumen de la Asignación</h3>
             <div className="border rounded-lg p-6 space-y-5 bg-zinc-900/30">
                    ...
                    <div className="text-right font-semibold">
                        {state.scheduleSelectionType === 'existing' ? (
                            ...
                            <div>
                                ...
                                <p className="text-xs text-muted-foreground">{state.newScheduleData.detalles.length} turnos definidos</p>
                            </div>
                        )}
                    </div>
                </div>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold">
                        {state.assignmentStartDate ? format(state.assignmentStartDate, "PPP", { locale: es }) : 'N/A'}
                    </span>
                </div>
                <div className="flex justify-between items-center">
                    <span className="text-muted-foreground">Fecha de Fin:</span>
                    <span className="font-semibold">
                        {state.assignmentEndDate ? format(state.assignmentEndDate, "PPP", { locale: es }) : 'Indefinida'}
                    </span>
                </div>
                ...
                <div className="flex justify-between items-center">
                    ...
                    <span className="font-semibold">{getScheduleTypeName(state.selectedScheduleTypeId)}</span>
                </div>
                <div className="flex justify-between items-center pt-4 border-t border-zinc-700">
                    <span className="text-muted-foreground font-semibold">Empleado:</span>
                    <span className="font-bold text-lg">{state.selectedEmployee?.nombreCompleto}</span>
                </div>
             </div>
        </div>
    );
    ...
    useEffect(() => {
        ...
        const loadAssignmentData = async () => {
            ...
            try {
                ...
                const employee: EmpleadoSimpleDTO = { id: data.empleadoId, nombreCompleto: data.empleadoNombre };
                ...
                toast({ title: "Error", description: `No se pudo cargar la asignación: ${error.message}`, variant: "destructive" });
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            if (state.scheduleSelectionType === 'new') {
                 throw new Error("No se puede crear una nueva plantilla en modo de edición.");
            }
            ...
            toast({ title: "Actualización Exitosa", description: "El horario ha sido actualizado." });
            router.push("/horarios/asignados");
    ...
    return (
        <div className="p-4 sm:p-6 lg:p-8 space-y-6">
             ...
             <div className="flex items-center justify-between">
                ...
                <Link href="/horarios/asignados"><Button variant="outline">Cancelar</Button></Link>
             </div>
                ...
                <Card>
                    ...
                    <CardContent className="min-h-[350px] p-6">
                        {state.step === 'selectSchedule' && (
                           <Step2_SelectSchedule state={state} dispatch={dispatch} setTemplates={setTemplates} isEditMode={true} />
                        )}
                        {state.step === 'setDates' && (
                            <Step3_SetDates state={state} dispatch={dispatch} setScheduleTypes={setScheduleTypes} />
                        )}
                        {state.step === 'summary' && (
