...
import { Trash2, Loader2, AlertCircle, User, Fingerprint, Info, Plus, ArrowRight } from "lucide-react"
...
export function EmployeeDetailsModal({ 
  ...
  const [fingerprintToDelete, setFingerprintToDelete] = useState<Huella | null>(null);
  ...
  const fetchFingerprints = useCallback(async () => {
    ...
    try {
      const response = await axios.get<Huella[]>(`${API_BASE_URL}/api/empleados/${employee.id}/huellas`);
  ...
  const handleDeleteFingerprint = async () => {
    ...
    try {
      await axios.delete(`${API_BASE_URL}/api/empleados/${employee.id}/huellas/${fingerprintToDelete.id}`);
      ...
      setError("Error al eliminar la huella. Inténtelo de nuevo.");
       toast({
        ...
        description: "No se pudo eliminar la huella. Por favor, inténtelo de nuevo.",
        variant: "destructive",
      });
    } finally {
  ...
  const handleFingerAction = (action: 'register' | 'delete') => {
    ...
    const huella = huellas.find(h => h.nombreDedo.toUpperCase() === fingerName);
  ...
  const registeredFingerIndices = huellas.map(h => FINGER_NAME_TO_INDEX[h.nombreDedo.toUpperCase()]).filter(Boolean);
  ...
  return (
    <>
        ...
        <div className="space-y-6 p-1">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
            <div className="space-y-1">
                <p className="text-sm font-medium text-zinc-400">Nombre Completo</p>
             ...
             <div className="space-y-1">
                <p className="text-sm font-medium text-zinc-400">Correo Institucional</p>
                <p>{employee.correoInstitucional || 'N/A'}</p>
            </div>
            <div className="space-y-1">
                <p className="text-sm font-medium text-zinc-400">Estado</p>
                <Badge
                  variant={employee.estatusNombre?.toLowerCase() === 'activo' ? 'default' : 'secondary'}
                  className={
              ...
              <div className="bg-zinc-800/50 p-4 rounded-lg">
                <div className="flex items-center gap-2 text-sm text-zinc-400 mb-4 p-2 bg-zinc-900/50 rounded-md">
                   <Info className="h-5 w-5" />
                    <span>Haz clic en cualquier dedo para ver opciones disponibles.</span>
                </div>
                <CompactHandSelector 
                      ...
                      {huellas.map((huella) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <span className="font-medium capitalize">{getFingerName(huella).toLowerCase()}</span>
                          </div>
                          <Button 
                            variant="ghost" 
                            size="sm" 
                      ...
                      >
                        ...
                        <div className="space-y-2">
                          ...
                          <p className="text-zinc-500 max-w-md mx-auto">
                            Este empleado aún no tiene huellas dactilares registradas en el sistema.
                          </p>
                        </div>
                        ...
                        <div className="text-xs text-zinc-600 max-w-sm mx-auto">
                          <p>
                            💡 <strong>Tip:</strong> Se recomienda registrar al menos 2 dedos diferentes para mayor seguridad
                          </p>
                        </div>
                      </motion.div>
                    )
      ...
      <AlertDialog open={showFingerActions} onOpenChange={setShowFingerActions}>
        <AlertDialogContent className="bg-zinc-900 border-zinc-800 text-white">
          ...
          <div className="space-y-2 py-4">
            {selectedFingerIndex && (() => {
              const fingerName = INDEX_TO_FINGER_NAME[selectedFingerIndex]?.toUpperCase();
              const isRegistered = fingerName && huellas.some(h => h.nombreDedo.toUpperCase() === fingerName);
      ...
      {/* --- Confirmation Dialog --- */}
      <AlertDialog open={!!fingerprintToDelete} onOpenChange={(open) => !open && setFingerprintToDelete(null)}>
          <AlertDialogContent className="bg-zinc-900 border-zinc-800 text-white">
            <AlertDialogHeader>
              <AlertDialogTitle className="flex items-center gap-2">
              ...
              <AlertDialogDescription className="text-zinc-400 pt-2">
                ...
                <strong className="text-red-400"> {fingerprintToDelete ? getFingerName(fingerprintToDelete).toLowerCase() : 'seleccionado'}</strong>?
                <br />
