...
function AsignarHuellaContent() {
  ...
  // 6) Cleanup al desmontar. Liberar lector y desconectar WebSocket
  useEffect(() => {
    return () => {
  ...
  const searchEmployees = useCallback(
    async (searchText: string) => {
      ...
      try {
        ...
        const resp = await axios.get<EmployeeSearchItem[]>(
          `${API_BASE_URL}/api/empleados?search=${encodeURIComponent(
            searchText
          )}`
        );
        setEmployeesFound(resp.data);
      } catch (error: any) {
        console.error("Error buscando empleados:", error);
        setEmployeesFound([]);
        setGeneralError("No se pudo buscar empleados.");
      } finally {
        setIsLoading(false);
      }
    },
  ...
  const goToPreviousStep = () => {
    ...
    if (currentStep === 5) {
      // Ir a step3 para registrar otro dedo
      // (o a step4 si quieres). Ajusta a tu gusto.
      // Por simplicidad iremos a step3
      setCurrentStep(3);
      resetCaptureProcess();
    }
          ...
          <CardContent>
            <div className="space-y-6">
              ...
              <div className="space-y-2">
                ...
                {selectedScanner && (
                  <div className="p-2 bg-blue-500/10 border border-blue-500/30 rounded-lg mt-3 text-blue-400 flex items-center gap-2">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 mb-2 bg-zinc-800/50 p-2 rounded-md">
                ...
                <div>
                  ...
                  <p className="text-xs text-zinc-400">Lector: {selectedScanner}</p>
                </div>
              </div>
          ...
          <Card className="bg-zinc-900 border-zinc-800">
              ...
              <div className="space-y-6">
                ...
                <div>
                  ...
                  <div className="flex justify-between items-center gap-2">
                    {Array.from({ length: ENROLLMENT_STEPS_NEEDED }).map(
                      (_, index) => {
                ...
                <div className="min-h-[3rem] p-3 bg-zinc-800 border rounded-md text-center flex items-center justify-center">
                  {captureError ? (
                    <p className="text-red-400">{captureError}</p>
                  ) : (
                    <p className="text-sm text-zinc-300">
                      {(captureState === "saving" ||
                        ...
                        <Loader2 className="inline animate-spin mr-1" />
                      )}
                      {captureFeedbackMsg || "..."}
                    </p>
                  )}
            ...
            <CardFooter className="flex justify-between">
              <Button
                ...
                disabled={isLoading || captureState === "saving"}
              >
                <ArrowLeft className="mr-2 h-4 w-4" />
                Volver
              </Button>
    ...
    <div className="p-6 md:p-8">
      ...
      <div className="mb-8 max-w-4xl mx-auto">
        <div className="flex items-center justify-between">
          {["Empleado", "Lector", "Dedo", "Captura", "Finalizado"].map(
            (label, index) => (
                ...
                {index < 4 && (
                  <div
                    className={`flex-1 h-0.5 mt-4 ${
                      currentStep > index + 1 ? "bg-blue-600" : "bg-zinc-700"
                    }`}
                  ></div>
                )}
              </React.Fragment>
