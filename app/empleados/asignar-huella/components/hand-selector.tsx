...
export const HandSelector = ({
  ...
  const getColor = (status: ReturnType<typeof getFingerStatus>) => {
    switch (status) {
      case 'hovered':
        return 'hsl(var(--primary))'; // Color primario (verde institucional)
      case 'selected':
        return 'hsl(var(--primary))'; // Color primario
      case 'session':
        return 'hsl(var(--accent))'; // Color accent (naranja institucional)
      case 'existing':
        return 'hsl(142 76% 36%)'; // Verde éxito
      default:
        return 'hsl(var(--muted-foreground))'; // Color muted
    }
  };
            ...
            {leftHandFingerRegions.map((finger) => (
              <motion.div
                ...
                animate={{
                  ...
                  scale:
                    getFingerStatus(finger.id) === 'selected' ||
                    getFingerStatus(finger.id) === 'hovered'
                      ? 1.1
                      : 1,
                }}
